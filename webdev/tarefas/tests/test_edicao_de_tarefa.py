...
def resposta_com_tarefa_pendente(client, tarefa_pendente):
    resp = client.post(reverse('tarefas:detalhe', kwargs={'tarefa_id': tarefa_pendente.id}), data={'feita': 'true', 'nome': f'{tarefa_pendente.nome}--editada'})
    return resp
...
def resposta_com_tarefa_feita(client, tarefa_feita):
    resp = client.post(reverse('tarefas:detalhe', kwargs={'tarefa_id': tarefa_feita.id}), data={'nome': f'{tarefa_feita.nome}--editada'})
    return resp
